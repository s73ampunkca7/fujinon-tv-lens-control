...
void loop() {
  ...
  while (portOne.available() > 0) 
  {
    ...
    if (firstRun && (data == 0xF3)) //Daten vorfiltern
    {    
      printOut = true;  
      //dataDetected = true; 
    }
    ...
    if (printOut)
    { 
      sprintf(hexStr, "%02X", data);//hex
      //sprintf(hexStr, "%03d", data); //dez
      //if (i>=7 && i<=7) //Blende filtern
      //if (i>=8 && i<=9) //Zoom filtern
      //if (i>=11 && i<=11) //Fokuswert filtern
      {
        //if (data != dataLast) //geht nur bei einem byte
        {
          dataLast = data;
          dataDetected = true; 
